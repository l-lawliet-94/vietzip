...
// `extern "system"`, not `"C"` — same fix and same reason as vendor/unrar-ng-sys (this is a
// different, older crate with the identical RARDLL C API and the identical `PASCAL`/`__stdcall`
// mismatch on x86; see that crate's comment for the full explanation).
...
// `extern "system"` — see the comment on `ChangeVolProc`/`ProcessDataProc`/`Callback` above.
extern "system" {
