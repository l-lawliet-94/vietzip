...
impl<M: ProcessMode> Internal<M> {
    // `extern "system"` — must match `unrar_sys::Callback`'s calling convention (patched in
    // vendor/unrar_sys for the same reason: RARSetCallback expects `PASCAL`/`__stdcall` on real
    // Windows x86; see that crate's comment for the full explanation).
