...
// `extern "system"` (not `"C"`): `vendor/unrar/dll.hpp` declares these as `PASCAL`/`CALLBACK`,
// which on real Windows resolve to `__stdcall` (the empty `#define CALLBACK`/`#define PASCAL`
// only apply under `#ifdef _UNIX`, confirmed by reading the header directly). On x86 this
// changes the decorated symbol name (MSVC emits `_Name@N` for stdcall vs. bare `_Name` for
// cdecl) and the actual argument-cleanup convention; `extern "C"` silently linked/worked on x64
// only because x64's calling convention is uniform (no cdecl/stdcall distinction at the ABI
// level), which masked this mismatch until an i686 (32-bit Windows) build was attempted and hit
// real `LNK2019 unresolved external symbol` errors for the unmangled cdecl names. `extern
// "system"` is Rust's standard cross-target idiom for exactly this: stdcall on Windows x86,
// identical to `extern "C"` everywhere else (including x64) — a correctness fix for x86, a
// verified no-op for every other target this project builds.
...
// `extern "system"`, not `"C"` — see the comment on `ChangeVolProc`/`ProcessDataProc`/`Callback`
// above: `dll.hpp` declares every one of these `PASCAL` (`__stdcall` on real Windows), which
// only matters on x86 (stdcall vs. cdecl name-decoration/argument-cleanup differ there; x64 has
// no such distinction, so this was invisible until an i686 build was attempted).
